...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a CARDbuyers address (e.g. %1)").arg("BL4YpGWPRWYW5LC8QhC3BbjToaTVTP8Waa"));
#endif
...
{
    // return if URI is not valid or is no CARDbuyers: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++) {
        ...
        if (i->first == "message") {
            ...
            if (!i->second.isEmpty()) {
                if (!BitcoinUnits::parse(BitcoinUnits::BCARD, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
{
    // Convert CARDbuyers:// to CARDbuyers:
    //
    //    Cannot handle this later, because CARDbuyers:// will cause Qt to see the part after // as host,
...
QString formatBitcoinURI(const SendCoinsRecipient& info)
{
    ...
    if (info.amount) {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BCARD, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open CARDbuyers.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CARDbuyers.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for CARDbuyers.lnk
...
{
    return GetAutostartDir() / "CARDbuyers.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a CARDbuyers.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=CARDbuyers\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the CARDbuyers app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add CARDbuyers app to startup item list
